...
int list_files(glob_t *pglob)
{
  ...
  check(glob_t != NULL, "Invalid glob_t given");
  check_mem(line);
  ...
  while (fgets(line, MAX_LINE - 1, file) != NULL)
  {
    ...
    // dumb work around to stupid design in glob
...
int found_it(int use_or, int found_count, int search_len)
{
  ...
  if (!use_or && found_count == search_len)
  {
    return 1;
  }
...
int scan_file(const char *filename, int use_or, int search_len, char *search_for[])
{
  ...
  // read each line and search that line for the contents
  while (fgets(line, MAX_LINE - 1, file) != NULL)
  {
    for (i = 0; i < search_len; i++)
    {
      if (strcasestr(line, search_for[i]) == 0)
      {
        debug("file: %s, line: %s, search: %s", file, line, search_for[i]);
        found_count++;
      }
    }
